...
| **[ADR-0002: Serverless Cloud Deployment](0002-cloud-deployment-free-tier.md)** | `Accepted` | Define a implantação na nuvem voltada para **custo zero (Free Tier)**. Hospeda a aplicação no **Google Cloud Run** (escala até zero), banco de dados no **Supabase PostgreSQL** (serverless/managed) e cache no **Upstash Redis** (serverless). |
...
| **[ADR-0018: Live Match Timeline & Commentary (Minuto a Minuto)](0018-live-match-timeline-and-commentary.md)** | `Accepted` | Estratégia de custo zero com ESPN Public API, paywall para assinantes, buffer Redis (20-30s) e persistência definitiva pós-jogo. |
